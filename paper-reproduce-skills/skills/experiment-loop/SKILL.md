...
## ポテンシャル最大化 (P0-E、`reimplement/SKILL.md` § 核心原則 参照)

inference / training attempt は **論文・公式コードのデフォルト引数を変更せずに起動する** のが既定。`action=run_demo`, `action=run_inference`, `action=run_train` の試行で `--num_inference_steps`, `--iterations`, `--num_frames`, `--resolution`, `--num_views`, `--num_samples` 等を README / `examples/` / `configs/*.yaml` の宣言値から**減らさない**。

縮小は OOM ladder / Arch Upgrade Ladder の正当な発動時のみ許可。`intent` に「OOM で frame 数を半減 (OOM ladder Step 2)」のような**物理的根拠**を明記する。`"to save time"` / `"for quick smoke"` / `"小さくして動作確認"` は intent として禁止 (= Tier 0 違反)。

実行時間は tier 判定に影響しない (= 数時間かかっても `tier0`/`tier1`/`tier2-config` を発動しない、claim 達成可否で判定する)。
