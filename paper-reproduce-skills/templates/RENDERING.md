...
| `{{T_H2_SUMMARY}}` / `{{T_H2_ENVIRONMENT}}` / `{{T_H2_OVERVIEW}}` / `{{T_H2_PROBLEM}}` / `{{T_H2_USAGE}}` / `{{T_H2_NEXT_ACTIONS}}` / `{{T_H2_SAMPLES}}` / `{{T_H2_ATTEMPTS}}` / `{{T_H2_ARTIFACTS}}` / `{{T_H2_PIXI}}` / `{{T_H2_ERRORS}}` / `{{T_H2_RELATED_ISSUES}}` | `h2_*` |
...
| 空状態メッセージ | `empty_overview` / `empty_problem` / `empty_environment` / `empty_quickstart` / `empty_advanced` / `empty_developer` / `empty_samples` / `empty_next_actions` / `empty_related_issues` |
