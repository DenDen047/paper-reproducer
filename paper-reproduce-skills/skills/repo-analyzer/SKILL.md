...
論文 Table N の数値 claim を `analysis.json.paper_claims[]` に抽出し、空 / 非空のいずれの場合も **理由 enum を `claims_extraction.status` に必須記録** する。これは P0-C (paper-claim-audit 連携) の前提であり、`reproduction_mode` 判定の起点でもある。
